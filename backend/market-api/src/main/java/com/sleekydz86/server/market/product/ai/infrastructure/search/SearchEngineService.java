package com.sleekydz86.server.market.product.ai.infrastructure.search;

import cn.hutool.json.JSONUtil;
import com.sleekydz86.server.market.product.ai.bean.SearchEngineResponse;
import com.sleekydz86.server.market.product.ai.domain.search.SearchResult;
import com.sleekydz86.server.market.product.ai.domain.search.SearchService;
...
public class SearchEngineService implements SearchService {
    ...
    private List<SearchResult> processSearchResults(List<com.sleekydz86.server.market.product.ai.bean.SearchResult> results) {
        if (results.isEmpty()) {
            return Collections.emptyList();
        }
        ...
        return results.stream()
                ...
                .sorted(Comparator.comparingDouble(com.sleekydz86.server.market.product.ai.bean.SearchResult::getScore).reversed())
                .map(this::convertToDomainResult)
                .toList();
    }
    
    private SearchResult convertToDomainResult(com.sleekydz86.server.market.product.ai.bean.SearchResult beanResult) {
